...
MOCK_QUIZ_JSON = json.dumps({
    ...
    "options": {"A": "c5.xlarge", "B": "r5.xlarge", "C": "t3.medium", "D": "p3.2xlarge"},
    "answer": "B",
...
async def test_generate_quiz_parses_json():
    with patch("services.llm._call_model", new=AsyncMock(return_value=MOCK_QUIZ_JSON)):
...
async def test_generate_quiz_strips_markdown_fence():
    fenced = f"```json\n{MOCK_QUIZ_JSON}\n```"
    with patch("services.llm._call_model", new=AsyncMock(return_value=fenced)):
        from services.llm import generate_quiz
...
async def test_generate_quiz_retries_on_bad_json():
    ...
    with patch("services.llm._call_model", side_effect=side_effect):
        from services.llm import generate_quiz
