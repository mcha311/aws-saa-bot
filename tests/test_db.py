...
async def test_quiz_attempt_and_wrong_note():
    from services.db import init_db, get_or_create_user, save_question, save_attempt, upsert_wrong_note, get_user_stats
    await init_db()
    user_id = await get_or_create_user(999, "quizuser")
    qid = await save_question(
        domain="EC2", content="Which instance is cheapest?",
        options_json=json.dumps({"A": "t2", "B": "c5", "C": "r5", "D": "p3"}),
        answer="A", explanation="t2 is the cheapest.",
    )
...
async def test_mastery_flow():
    from services.db import init_db, get_or_create_user, save_question, upsert_wrong_note, update_mastery
    await init_db()
    user_id = await get_or_create_user(777, "masteryuser")
    qid = await save_question("S3", "q?", json.dumps({"A": "a", "B": "b", "C": "c", "D": "d"}), "A", "exp")
    await upsert_wrong_note(user_id, qid)
