...
class Stats(commands.Cog):
    ...
    async def stats(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            embed.add_field(name="총 문제", value=str(total), inline=True)
            embed.add_field(name="정답", value=str(correct), inline=True)
            embed.add_field(name="오답", value=str(wrong), inline=True)
            embed.add_field(name="전체 정답률", value=f"{_bar(accuracy)} {accuracy:.1f}%", inline=True)
            embed.add_field(name="오답 노트 잔여", value=f"{s['unmastered_wrong']}문제", inline=True)
            embed.add_field(name="학습 스트릭", value=f"🔥 {s['streak']}일 연속", inline=True)
            if s["domain_stats"]:
                lines = []
                for domain, ds in s["domain_stats"].items():
                    acc = (ds["correct"] / ds["total"] * 100) if ds["total"] > 0 else 0.0
                    lines.append(f"`{domain:<12}` {_bar(acc)} {acc:.0f}%  ({ds['total']}문제)")
                embed.add_field(name="도메인별 정답률", value="\n".join(lines), inline=False)
            await interaction.followup.send(embed=embed)
        except Exception as exc:
            import logging, traceback
            logging.getLogger(__name__).error("stats error: %s", traceback.format_exc())
            await interaction.followup.send(f"오류가 발생했습니다: {exc}", ephemeral=True)

    @app_commands.command(name="leaderboard", description="서버 Top 5 학습 랭킹을 봅니다")
    async def leaderboard(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            medals = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣"]
            lines = [
                f"{medals[i]} **{u['username']}** — {_bar(u['correct']/u['total']*100 if u['total'] else 0, 6)} "
                f"{u['correct']/u['total']*100 if u['total'] else 0:.1f}% ({u['total']}문제)"
            ...
            import logging, traceback
            logging.getLogger(__name__).error("leaderboard error: %s", traceback.format_exc())
            await interaction.followup.send(f"오류가 발생했습니다: {exc}", ephemeral=True)
