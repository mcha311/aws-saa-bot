...
class ReviewView(discord.ui.View):
    ...
    def _make_callback(self, option: str):
        async def callback(interaction: discord.Interaction) -> None:
            ...
            embed.add_field(name="해설", value=self.note["explanation"][:1020], inline=False)
    ...
    async def review(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            if not notes:
                embed = discord.Embed(
                    title="📭 오답 노트가 비어있어요",
                    description="/quiz를 풀다 보면 오답이 쌓입니다.\n그러면 여기서 복습할 수 있어요!",
                    color=discord.Color.green(),
                )
                await interaction.followup.send(embed=embed, ephemeral=True)
                return
            ...
            logging.getLogger(__name__).error("review error: %s", traceback.format_exc())
